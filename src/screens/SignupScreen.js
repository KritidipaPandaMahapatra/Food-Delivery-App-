...
const SignupScreen = ({navigation}) => {
       ...
       try{
        ...
        .then((userCredentials)=>{
          ...
          if(userCredentials?.user.uid){
            ...
            userRef.add({ name:name,
                email:email,
                password:password,
                phone:phone,
                address:address,
                uid:userCredentials?.user.uid}).then(()=>{
  ...
  return (
    ...
    <View style={styles.inputout}>
        <Feather name={'user'} size={24} color={namefocus === true? colors.text1 : colors.text2}/>
